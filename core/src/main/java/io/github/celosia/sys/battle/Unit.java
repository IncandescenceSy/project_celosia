...
public class Unit {
	...
	// Secondary effect block; >= 1 means blocks secondary effects the same as
	// Barrier
	...
	public void setDurationModBuffTypeDealt(BuffType buffType, int durationModBuffTypeDealt) {
		if (buffType == BuffType.BUFF)
			durationModBuffDealt = durationModBuffTypeDealt;
		else
			durationModDebuffDealt = durationModBuffTypeDealt;
	}
	...
	public void setDurationModBuffTypeTaken(BuffType buffType, int durationModBuffTypeTaken) {
		if (buffType == BuffType.BUFF)
			durationModBuffTaken = durationModBuffTypeTaken;
		else
			durationModDebuffTaken = durationModBuffTypeTaken;
	}
		...
		if (shield != 0 && --shieldTurns <= 0) {
			if (defend == 0)
				appendToLog(formatName(unitType.getName(), pos, false) + " " + lang.get("log.loses") + " " + c_shield
						+ String.format("%,d", shield / STAT_FACTOR) + " " + c_buff + lang.get("shield"));
			else
				appendToLog(formatName(unitType.getName(), pos) + " " + c_buff + lang.get("shield") + " " + c_shield
						+ String.format("%,d", (shield + defend) / STAT_FACTOR) + "[WHITE] → " + c_shield
						+ String.format("%,d", defend / STAT_FACTOR) + "[WHITE]/" + c_shield
						+ String.format("%,d", statsDefault.getHp()) + c_neg + " ("
						+ String.format("%,d", shield / STAT_FACTOR) + ")");
			shield = 0;
		}
	...
	public Result damage(int dmg, boolean pierce, boolean useName) {
		...
		if (!pierce) { // Pierce skips Defend and Shield
				...
				if (defend > dmg) { // Only hit Defend
					...
					return new Result(ResultType.HIT_SHIELD,
							name_s + lang.get("shield") + " " + c_shield
									+ String.format("%,d", (defendOld + shield) / STAT_FACTOR) + "[WHITE] → " + c_shield
									+ String.format("%,d", (defend + shield) / STAT_FACTOR) + "[WHITE]/" + c_shield
									+ String.format("%,d", statsDefault.getDisplayHp()) + "[WHITE] (" + c_neg + "-"
									+ String.format("%,d", dmgFull / STAT_FACTOR) + "[WHITE])");
				} else { // Destroy Defend and proceed to Shield
					dmg -= defend;
					defend = 0;
					if (shield == 0 && effectBlock <= 0)
						msg.add(name + lang.get("log.is_no_longer") + " " + c_buff + lang.get("log.effect_block")); // todo
																													// this
																													// should
																													// come
																													// after
			...
			if (shield > 0 && dmg > 0) { // There's Shield and dmg
				if (shield > dmg) { // Only hit Shield
					...
					return new Result(ResultType.HIT_SHIELD,
							name_s + lang.get("shield") + " " + c_shield
									+ String.format("%,d", (defendOld + shieldOld) / STAT_FACTOR) + "[WHITE] → "
									+ c_shield + String.format("%,d", shield / STAT_FACTOR) + "[WHITE]/" + c_shield
					...
					msg.add(name_s + lang.get("shield") + " " + c_shield
							+ String.format("%,d", (defendOld + shield) / STAT_FACTOR) + "[WHITE] → " + c_num + 0
							+ "[WHITE]/" + c_shield + statsDefault.getDisplayHp() + "[WHITE] (" + c_neg + "-"
							+ String.format("%,d", (defendOld + shield) / STAT_FACTOR) + "[WHITE])");
					dmg -= shield;
					shield = 0;
					shieldTurns = 0;
					if (effectBlock <= 0)
		...
		msg.add(name_s + c_stat + lang.get("hp") + " " + c_hp + String.format("%,d", hpOldDisp) + "[WHITE] → " + c_hp
				...
				+ String.format("%,d", (dmg / STAT_FACTOR)) + "[WHITE])");
