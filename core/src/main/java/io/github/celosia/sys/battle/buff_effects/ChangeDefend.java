...
public class ChangeDefend implements BuffEffect {
    ...
    public String[] onGive(Combatant self) {
        ...
        return new String[]{self.getCmbType().getName() + "'s " + lang.get("shield") + " " + String.format("%,d", (self.getShield() + defendOld)) + " -> " + String.format("%,d", (self.getShield() + defendNew)) +
            "/" + String.format("%,d", hpMax) + " (+" + String.format("%,d", ((self.getShield() + defendNew) - (self.getShield() + defendOld))) + ")",
            ((self.getEffectBlock() <= 0 && self.getShield() == 0 && defendOld == 0) ? self.getCmbType().getName() + " " + lang.get("log.is_now") + " " + lang.get("log.effect_block") : "")};
    }
    ...
    public String[] onRemove(Combatant self) {
        int defendOld = self.getDefend();
        self.setDefend(0);
        return new String[]{self.getCmbType().getName() + "'s " + lang.get("shield") + " " + String.format("%,d", (self.getShield() + defendOld)) + " -> " + String.format("%,d", self.getShield()) + "/" +
            String.format("%,d", self.getStatsDefault().getHp()) + " (" + String.format("%,d", (self.getShield() - (self.getShield() + defendOld))) + ")",
        ((self.getEffectBlock() <= 0 && self.getShield() == 0) ? self.getCmbType().getName() + " " + lang.get("log.is_no_longer") + " " + lang.get("log.effect_block") : "")};
    }
}
