...
public class Move {
    ...
    public boolean isValid() {
        ...
        if((!skill.getRange().isCanTargetSelf()) && (targetPos == self.getPos())) return false;

        // Check if target is within vertical range
        if(Math.abs(getHeight(self.getPos()) - getHeight(targetPos)) > skill.getRange().getRangeVertical()) return false;

        // Check if the targeted side is allowed
        return skill.getRange().getSide() == Side.BOTH || skill.getRange().getSide() == getRelativeSide(self.getPos(), targetPos);
    }
}
